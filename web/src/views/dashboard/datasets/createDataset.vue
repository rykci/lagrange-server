...
export default defineComponent({
        ...
        submitForm (formName) {
            that.$refs[formName].validate(async valid => {
                if (valid) {
                    ...
                    formData.append('data_schema', that.ruleForm.license)
                    const listRes = await that.system.$commonFun.sendRequest(`${process.env.VUE_APP_BASEAPI}dataset`, 'post', formData)
                    await that.system.$commonFun.timeout(500)
                    that.system.$commonFun.messageTip('success', 'Created Dataset successfully!')
                    that.$router.push({name: 'datasets'})
    ...
    activated() {
        that = this
        that.ruleForm.name = ''
        that.ruleForm.license = ''
    },
    computed: {
        metaAddress () {
            let val = this.$store.state.metaAddress || ''
